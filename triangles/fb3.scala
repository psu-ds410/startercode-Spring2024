...
object FB {
   ...
   def toyGraph(sc: SparkContext): RDD[(String, String)] = {
    ...
    def countTriangles(edgeList: RDD[(String, String)]) = {
        ...
        val no_self_edges =  edgeList.filter{case (a,b) => a != b}
        val redundant = makeRedundant(no_self_edges)
        ...
        val paths2 = redundant.join(redundant)
        val no_self_join = paths2.filter{case (a, (b,c)) => b != c}
        // rdd looks like this
        // (4,(3,3))    <-- was created by (4, 3) joined (4, 3)                                                                        // (4,(3,1))    <-  was created by (4, 3) joining (4, 1) there is a path from 3 to 4 to 1
        // (4,(1,3))    <-  there is a path from 1 to 4 to 3 
        // (4,(1,1))    <-  path from 1 to 4 to 1 (boring)
        // in general:
        // (a, (b, c)) <-  path from b to a to c (a is midpoint, values are the extremes)

        // now we need to check if there is an edge between (b, c). If so, (a, (b, c)) would be a triangle
        // we want to join a record (a, (b,c)) from no_self_join to (b,c) in 'redundant'
        //   but spark rdds only join on keys, so we change what the keys are.
        
        val rekey_join = no_self_join.map{case (a, (b,c)) => ((b,c), a)}
        val rekey_graph = redundant.map{case (b, c) => ((b,c), 'x')}
        val mega_join = rekey_join.join(rekey_graph)
        // values here look like this:
        //  ((1,3),(4,'x'))  this means ((1,3), 4) joined with ((1,3), 1)
        //                        but ((1,3), 4) means there is a path from 1 to 4 to 3
        //                        and ((1,3), 'x') means there is a edge (1,3)
        // this means 1 to 4 to 3 to 1 is a triangle
        // but ((4,3), (1, 'x')) represents the same triangle
        // ((3, 4), (1, 'x'))
        // ((1, 4), (3, 'x'))
        // ((4, 1), (3, 'x'))
        // we want  canonical form where ((a, b), (c, 'x')) and a < b < c
        val canonical = mega_join.filter{case ((a,b), (c, d)) => a < b && b < c} // removes everything not in the good form
        // we noticed duplilcates, so distinct
        canonical.count()
